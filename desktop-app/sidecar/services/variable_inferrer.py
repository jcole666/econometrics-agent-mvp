...
from sidecar.schemas import InferVariablesRequest, InferVariablesResponse
from sidecar.services.maas_client import MaasUnavailable, llm_provider_name, maas_chat

SYSTEM_PROMPT = """你负责根据研究问题和字段画像识别计量模型变量。
只返回 JSON，字段包括 dependent_variable, independent_variables, entity_column,
time_column, treatment_column, running_variable, instrument_variable, reasoning。
所有变量名必须来自用户提供的字段列表。reasoning 用中文。"""
...
def infer_variables(request: InferVariablesRequest) -> InferVariablesResponse:
    ...
    try:
        ...
        fallback.maas_error = f"MaaS 变量识别结果不可用，已使用本地规则。{exc}"
        return fallback
...
def _build_messages(request: InferVariablesRequest) -> list[dict[str, str]]:
    return [
        ...
        {"role": "user", "content": json.dumps(_dump_model(request), ensure_ascii=False)},
    ]
...
def _infer_variables_rules(request: InferVariablesRequest) -> InferVariablesResponse:
    ...
    entity_column = _find_first(columns, lower_names, ["id", "entity", "person", "firm", "company", "city", "province", "个体", "城市", "公司"])
    ...
    if treatment_column and treatment_column != dependent_variable and treatment_column not in independent_variables:
        independent_variables.append(treatment_column)

    reasoning = "已根据研究问题关键词和字段名进行本地规则识别。"
    if dependent_variable is None:
        reasoning = "未能稳定识别被解释变量 Y，请手动确认。"
    ...
    return InferVariablesResponse(
        ...
        running_variable=running_variable if _mentions_any(question, ["rdd", "断点", "阈值", "cutoff"]) else None,
...
def _guess_dependent_variable(columns: list[str], lower_names: dict[str, str], question: str) -> str | None:
    groups = [
        (["income", "wage", "salary", "earnings", "收入", "工资", "薪资"], ["income", "wage", "salary", "pay", "earnings"]),
        (["default", "违约"], ["default", "违约"]),
        (["score", "grade", "achievement", "成绩", "分数"], ["score", "grade", "成绩", "分数"]),
        (["admit", "admission", "录取"], ["admit", "admission", "录取"]),
    ...
    for name in columns:
        if not _is_obvious_identifier(name, lower_names[name]):
            return name
    return None


def _normalize_response(raw: dict[str, Any], request: InferVariablesRequest, provider: str) -> InferVariablesResponse:
    ...
    for item in raw.get("independent_variables", []):
        name = _valid_name(item, valid)
        if name and name != dependent_variable:
            independent_variables.append(name)
    ...
    return InferVariablesResponse(
        ...
        reasoning=str(raw.get("reasoning") or "MaaS 已返回变量识别结果，建议人工确认后使用。").strip(),
        provider=provider,
    )
